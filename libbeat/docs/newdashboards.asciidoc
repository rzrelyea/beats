...
The Beat index pattern is generated from the `etc/fields.yml` (version 5.0) or the `_meta/fields.yml` (version 5.1 and above), where all the fields for a Beat are defined. For each field, besides the `type`, you can configure the
